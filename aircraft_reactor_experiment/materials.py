...
def create_materials(operating_temp, c_temp = 977.59, fuel_temp = 977.59, Tm = 977.59):
    ...
    salt = openmc.Material(name='salt', temperature = fuel_temp)
    ...
    salt.set_density('g/cm3',3.79-0.00093*fuel_temp)

    #######################################################################
    #moderator blocks
    BeO = openmc.Material(name='BeO',temperature = Tm)

    # by ao, expressed in ppm
    BeO.add_element('Be',499792)
    BeO.add_nuclide('O16',499792)
    ...
    temp_adj_BeO = 1/(1+lambda_BeO*(Tm+253))
    ...
    inconel.add_nuclide('C0',0.035,percent_type='wo')
    ...
    #######################################################################
    #coolant
    coolant = openmc.Material(name='coolant',temperature = c_temp)
    coolant.add_element('Na',1.0)
    coolant.set_density('g/cm3',-0.00024*c_temp+0.88648)
    ...
    stainless.add_nuclide('C0',0.030,percent_type='wo')
    ...
    #stainless https://www.aesteiron.com/sa240-304l-stainless-steel-sheet-plate.html
    part = openmc.Material(name='part',temperature = operating_temp)
    part.add_nuclide('C0',0.030,percent_type='wo')
    part.add_element('Mn',2.00,percent_type='wo')
    part.add_element('P',0.045,percent_type='wo')
    part.add_element('S',0.030,percent_type='wo')
    part.add_element('Si',0.75,percent_type='wo')
    part.add_element('Cr',18.00,percent_type='wo')
    part.add_element('Ni',8.0,percent_type='wo')
    part.add_element('N',0.1,percent_type='wo')
    part.add_element('Fe',39.045,percent_type='wo')
    part.set_density('g/cm3',8.5*temp_adj_inconel)

    #stainless https://www.aesteiron.com/sa240-304l-stainless-steel-sheet-plate.html
    regulating = openmc.Material(name='regulating',temperature = operating_temp)
    regulating.add_nuclide('C0',0.030,percent_type='wo')
    regulating.add_element('Mn',2.00,percent_type='wo')
    regulating.add_element('P',0.045,percent_type='wo')
    regulating.add_element('S',0.030,percent_type='wo')
    regulating.add_element('Si',0.75,percent_type='wo')
    regulating.add_element('Cr',18.00,percent_type='wo')
    regulating.add_element('Ni',8.0,percent_type='wo')
    regulating.add_element('N',0.1,percent_type='wo')
    regulating.add_element('Fe',39.045,percent_type='wo')
    regulating.set_density('g/cm3',8.5*temp_adj_inconel)


    mats = [salt,BeO,inconel,insulation,coolant,helium,stainless,boron,part,
            regulating]
